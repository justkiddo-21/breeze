...
W07 removed the expired SSO refresh-token JSON compatibility path. The SSO
exchange installs refresh authority only through its HttpOnly cookie and never
returns a refresh token in JSON, regardless of environment configuration.
