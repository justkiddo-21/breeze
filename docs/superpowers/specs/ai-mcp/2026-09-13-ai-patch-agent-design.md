...
4. **Failed patch work** (new): `patch_job_results` rows in `failed` grouped by `(device, patch, errorMessage-class)` with attempt counts and last attempt time — the input the chase loop needs, which no sweep kind provides.
5. **Reboot backlog**: `devices.pending_reboot = true` plus the device's resolved reboot policy and next maintenance window, if any.
...
**There is nothing to route today.** `alerts` has no `category` column (category lives on `alert_templates`), no patch service emits an alert, and `AiAgentTriggers` has no category filter — only `alertRuleIds` (`types/aiAgents.ts:196-202`). So item 3 requires first authoring patch alert sources (the `patch_compliance` monitor definition type, `db/schema/monitorDefinitions.ts:39`, plus new templates for patch-job failure and reboot-pending-over-threshold), then routing.
...
- **Run now** — `POST /ai/agents/:id/runs` with `{ profile: 'patch', orgId }` (device-less), mirroring `POST /ai/fleet-design/runs`; the existing device-scoped body stays. First web caller of the endpoint; `runAction` wrapped; MFA as today.
...
**No new tables.** One idempotent migration, `apps/api/migrations/2026-10-16-180700-ai-agents-patch-profile.sql` (sorts after main's newest `2026-10-16-180200-…`; 180300/180500 are taken by in-flight PRs). DDL only, no DML, so no `breeze.scope` election:
...
- **A (recommend)** — defer to W04 and author patch alert sources first (`patch_compliance` monitor definition + patch-job-failure and reboot-pending templates), then route with exclusive patch-work ownership and recorded fallback.
- B — ship routing now on `alertRuleIds` the partner authors by hand.
**Recommend A**; today there is literally no patch alert to react to.
...
|---|---|---|
| **W01 — the lane, end to end** | schedule kind + run profile + kind-gate map + org-listing fix + `patchEvidence.ts` + `submit_patch_plan` + safe projection + digest + default cadence **with backfill** + schedules UI for patch agents + next-occurrence on the card. **Findings only, zero intents.** | medium (new autonomous LLM lane; no mutations) |
| **W02 — actionable installs** | shared eligibility resolver extraction, device-scoped Tier-3 install proposals, episode-derived idempotency + suppression, release revalidation, Run now. | **high** — approvals, tenancy, unattended-adjacent. Full rigor. |
