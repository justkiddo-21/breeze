...
describe('buildInstallCommands', () => {
  ...
  describe('Windows (PowerShell)', () => {
    ...
    it('downloads the agent from ghBase (our signed release), not the API route', () => {
      ...
      const { windows } = buildInstallCommands(base);
      expect(windows).toContain(
        `Invoke-WebRequest -Uri "${base.ghBase}/breeze-agent-windows-amd64.exe" -OutFile $exe`
      );
    ...
    it('downloads the watchdog as a sibling only when watchdogUrl is provided, before service install', () => {
      ...
      expect(withWatchdog.windows).toContain("-OutFile (Join-Path $d 'breeze-watchdog.exe')");
