...
> = {
  ...
  agent: {
    label: "Agent",
    icon: Wifi,
    color: "border-teal-500/30 bg-teal-500/10 text-teal-600",
  },
  script: {
    label: "Script",
    icon: Terminal,
  ...
  policy: {
    label: "Policy",
    icon: Shield,
    color: "border-indigo-500/30 bg-indigo-500/10 text-indigo-600",
  },
  deployment: {
    label: "Deployment",
    icon: Package,
    color: "border-cyan-500/30 bg-cyan-500/10 text-cyan-600",
  },
  software: {
    label: "Software",
    icon: Package,
  ...
  discovery: {
    label: "Discovery",
    icon: Wifi,
    color: "border-sky-500/30 bg-sky-500/10 text-sky-600",
  },
  automation: {
    label: "Automation",
    icon: Activity,
    color: "border-fuchsia-500/30 bg-fuchsia-500/10 text-fuchsia-600",
  },
  maintenance: {
    label: "Maintenance",
    icon: Wrench,
  ...
  monitoring: {
    label: "Monitoring",
    icon: Activity,
    color: "border-lime-500/30 bg-lime-500/10 text-lime-600",
  },
  ai: {
    label: "AI",
    icon: Bot,
...
> = {
  manual: {
    label: "Manual",
    icon: User,
    color: "border-gray-400/30 bg-gray-400/10 text-gray-500",
  },
  ai: {
    label: "AI",
    icon: Bot,
...
export default function DeviceEventLogViewer({
  deviceId,
  timezone,
}: DeviceEventLogViewerProps) {
  ...
  const fetchActivities = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchWithAuth(
        `/devices/${deviceId}/events?${params}`,
      );
      ...
      setError(
        err instanceof Error
          ? err.message
          : t("deviceEventLogViewer.failedToFetchActivities"),
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div className="relative flex-1 max-w-sm">
              ...
              <input
                type="text"
                placeholder={t("deviceEventLogViewer.searchActivities")}
                value={searchQuery}
            ...
            <div className="flex items-center rounded-md border bg-background">
              {[
                { value: "", label: t("deviceEventLogViewer.all") },
                { value: "success", label: t("deviceEventLogViewer.success") },
                { value: "failure", label: t("deviceEventLogViewer.failed") },
              ].map((opt) => (
                <button
                  key={opt.value}
                  type="button"
        ...
        {loading ? (
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="mt-3 text-sm text-muted-foreground">
                {t("deviceEventLogViewer.loadingActivities")}
              </p>
            </div>
          </div>
        ) : activities.length === 0 ? (
          <div className="py-12 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {t("deviceEventLogViewer.noActivitiesMatchTheCurrentFilters")}
            </p>
          </div>
        ) : (
                ...
                <div key={activity.id} className="group">
                  ...
                  >
                    ...
                    <div className="mt-1.5">
                      <span
                        className={`inline-block h-2 w-2 rounded-full ${rc.dot}`}
                      />
                    </div>
                      ...
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="flex shrink-0 items-center gap-2">
                          ...
                          {activity.initiatedBy &&
                            initiatedByConfig[activity.initiatedBy] &&
                            (() => {
                              const ib =
                                initiatedByConfig[activity.initiatedBy!];
                        ...
                        <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-xs sm:grid-cols-3 lg:grid-cols-4">
                          <div>
                            <span className="font-medium text-muted-foreground">
                              {t("deviceEventLogViewer.action")}
                            </span>
                            <p className="mt-0.5 font-mono chart-legend-xs">
                              {activity.action}
                            </p>
                          </div>
                          <div>
                            <span className="font-medium text-muted-foreground">
                              {t("deviceEventLogViewer.result")}
                            </span>
                          ...
                          <div>
                            <span className="font-medium text-muted-foreground">
                              {t("deviceEventLogViewer.timestamp")}
                            </span>
                            <p className="mt-0.5">
                              {formatDateTime(
                                activity.timestamp,
                          ...
                          {activity.resource.type && (
                            <div>
                              <span className="font-medium text-muted-foreground">
                                {t("deviceEventLogViewer.resourceType")}
                              </span>
                          ...
                          {activity.resource.name && (
                            <div>
                              <span className="font-medium text-muted-foreground">
                                {t("deviceEventLogViewer.resource")}
                              </span>
                          ...
                          {activity.ipAddress && (
                            <div>
                              <span className="font-medium text-muted-foreground">
                                {t("deviceEventLogViewer.ipAddress")}
                              </span>
                              <p className="mt-0.5 font-mono chart-legend-xs">
                                {activity.ipAddress}
                              </p>
                          ...
                          Object.keys(activity.details).length > 0 && (
                            <div>
                              <span className="text-xs font-medium text-muted-foreground">
                                {t("deviceEventLogViewer.details")}
                              </span>
