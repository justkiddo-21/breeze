...
<Aside type="note">
  Non-file snapshots are badged by type in the browser — a Windows system-state snapshot shows a **System image** badge in both the snapshot selector and the detail header, so you can tell it apart from a file backup at a glance.
</Aside>
...
| **Bare metal recovery** | System-state artifacts and/or files onto a machine that reinstalls or boots its own OS | Replacing a dead machine entirely. See [Bare Metal Recovery](/backup/bare-metal-recovery/) |
