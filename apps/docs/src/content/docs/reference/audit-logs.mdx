...
<Aside type="caution">
  Both the read and write endpoints require a **partner- or system-scoped** session in addition to the `audit:manage`/`audit:read` permission — an Org Admin user token is org-scoped and gets a 403 even though Org Admin holds those permissions by default. In practice, only a partner-level user (or a platform admin) can currently set an organization's audit retention policy.
</Aside>
