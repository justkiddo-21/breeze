...
There are two possible outcomes, and which one happens is not under the user's control -- it depends on what the phone's hardware supports:

- **Hardware-attested.** On a phone with a Secure Enclave (iOS) or StrongBox/TEE (Android), the app generates a hardware-backed key and proves it with a platform attestation (App Attest on iOS, Play Integrity on Android). This device can satisfy up to **L4** -- the assurance floor for critical-risk approvals.
- **Legacy, unattested.** On a phone without that hardware, the app instead registers a biometric-gated key with no attestation behind it. Breeze is honest about the difference: an unattested device is capped at **L2/L3**, and cannot approve anything requiring L4.
