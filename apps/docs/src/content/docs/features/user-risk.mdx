...
<Aside type="caution">
  The `siteId` filter is opt-in only: a site-restricted technician who omits it still sees risk scores for every user in the organization, not just their assigned site. Site-scoped visibility for this feature is not yet enforced -- tracked in issue [#5428](https://github.com/LanternOps/breeze/issues/5428).
</Aside>
