...
The service-insight sections — Dashboard, Security, Backups, Reports, and support usage — are controlled separately, per organization, and every one of them starts **off**. You decide which customers get to see what.
...
3. In the **Visibility** panel, turn on the sections this customer should see. **Enable all visibility** turns on all five at once.
...
The device list is governed by the existing **Self-service** feature toggle, not by the visibility settings.
