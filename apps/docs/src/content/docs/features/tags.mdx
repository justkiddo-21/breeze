...
<Aside type="note">
  Manual assets (network gear, printers, and other equipment tracked without an agent) have their own separate `tags` array and are not counted here -- `GET /api/v1/tags` only aggregates tags from the `devices` table.
</Aside>
