...
Contract access is governed by role permissions: viewing requires contract read access, creating and editing requires contract write access, and lifecycle actions (activate, pause, resume, cancel) require contract management access. The same permissions gate the contract template library and service deliverables. Attaching evidence to a deliverable additionally needs the **Documents: write** permission (`documents:write`), because evidence is filed in the organization's document library. Assign these through **Settings → Users & Roles**.
