...
<Aside>
  The Helper requires the Breeze agent to be enrolled and connected. It authenticates to the API using its own helper-scoped `brz_`-prefixed token — issued alongside, but distinct from, the agent's own token — and communicates with the agent service over a local IPC socket.
</Aside>
