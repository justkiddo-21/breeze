...
import {
  ...
  initializeMetricAnomalyIncidentRetention,
  shutdownMetricAnomalyIncidentRetention,
} from './metricAnomalyIncidentRetention';
...
describe('Metric anomaly incident retention worker', () => {
  ...
  afterEach(async () => {
    warnSpy.mockRestore();
    await shutdownMetricAnomalyIncidentRetention();
  });

  it('registers a repeatable pruning job carrying batchSize/maxBatches', async () => {
    await initializeMetricAnomalyIncidentRetention();

    expect(attachWorkerObservabilityMock).toHaveBeenCalledWith(expect.anything(), 'metricAnomalyIncidentRetention');
  ...
  it('deletes metric_anomaly_incidents in bounded ctid batches, inside system DB context', async () => {
    ...
    expect(sqlDump).toContain('dispatched_at');
    expect(sqlDump).toContain('dispatch_attempts');
  ...
  it('stops at maxBatches, reports hasMore, and warns about the backlog', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledTimes(1);
    expect(warnSpy.mock.calls[0][0]).toContain('metric_anomaly_incidents');
  });
