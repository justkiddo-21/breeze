...
import {
  __testOnly,
  createTicketOutboxRetentionWorker,
  initializeTicketOutboxRetention,
  shutdownTicketOutboxRetention,
} from './ticketOutboxRetention';
...
describe('Ticket outbox retention worker', () => {
  ...
  afterEach(async () => {
    warnSpy.mockRestore();
    await shutdownTicketOutboxRetention();
  });

  it('registers a repeatable pruning job carrying batchSize/maxBatches', async () => {
    await initializeTicketOutboxRetention();

    expect(attachWorkerObservabilityMock).toHaveBeenCalledWith(expect.anything(), 'ticketOutboxRetention');
  ...
  it('deletes ticket_outbox in bounded ctid batches, inside system DB context', async () => {
    ...
    expect(withSystemDbAccessContextMock.mock.calls.map((c) => c[1])).toEqual([
      ...Array(3).fill('ticketOutboxRetention.prune.delivered'),
      'ticketOutboxRetention.prune.stuck',
    ]);
    ...
    expect(sqlDump).toContain('ticket_outbox');
  ...
  // so a stuck-row purge is visible on its own — see pruneTicketOutbox's doc
  ...
  it('stops at maxBatches, reports hasMore, and warns about the backlog', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledTimes(1);
    expect(warnSpy.mock.calls[0][0]).toContain('ticket_outbox');
  });
