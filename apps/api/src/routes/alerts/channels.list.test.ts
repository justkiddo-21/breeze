...
// Sweep 2026-09-08 (G6-4) — GET /alerts/channels?orgId=<org> for a
// partner-scoped caller built the per-org filter as a bare
// `eq(notificationChannels.orgId, query.orgId)`, unlike the sibling "all
// orgs" branch right below it (no ?orgId=) which already ORs in the
// partner's own partner-wide channels (org_id NULL, partner_id = auth.
// partnerId, #2130). A channel created via POST /alerts/channels with
...
vi.mock('../../services/notificationChannelSecrets', () => ({
  ...
  scrubChannelTestError: vi.fn(() => null),
}));
 ...
 * its static text. The mocked schema columns above are plain objects (not
 * real drizzle Column instances), so `eq`/`isNull` embed both the column
 * object and the raw bound value directly into `queryChunks` rather than
 * wrapping either in a Param — walking the tree recovers exactly what the
 * route asked for. Same introspection approach as
 * automationWorker.resolveAssignment.test.ts / ticketSlaWorker.test.ts.
 */
...
describe('GET /alerts/channels?orgId= — partner-wide read branch (sweep G6-4)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('includes the partner-wide branch for a PARTNER-scoped caller with ?orgId=', async () => {
    ...
    const res = await makeApp().request(`/alerts/channels?orgId=${ORG_ID}`);
    expect(res.status).toBe(200);

    const listText = sqlText(listWhereRef.current);
    expect(listText).toContain('org_id');
    expect(listText).toContain(ORG_ID);
    // The dual-axis branch: org_id IS NULL AND partner_id = <caller's partner>.
    expect(listText).toContain('is null');
    expect(listText).toContain('partner_id');
    expect(listText).toContain(PARTNER_ID);

    // Total must reflect the same condition the list used (pagination parity).
