-- 2026-10-13-100300: export offline_behavior in the canonical partner projection (#5128 W3).
--
-- 2026-10-13-100100 added config_policy_patch_settings.offline_behavior and the
-- API added 'offlineBehavior' to PATCH_NORMALIZED_MATERIAL_KEYS
-- (routes/partnerApi/configuration.ts). That key set is checked for EXACT
-- membership and exact cardinality by canonicalizePolicyPatchSettings, so a
-- column the SQL projection does not emit does not merely go missing: the whole
-- policy fails closed and is reported as `blocked: secret_detected`. That is
-- what reddened patchCanonicalExportParity and partnerApiConfigurationWatermark
-- on the W3 branch.
--
...
-- this reproduces the whole body from 2026-10-05-100200-patch-reboot-deferral-
-- settings.sql, which is the currently authoritative definition, with one key
-- added.
--
  ...
  CASE feature_type
    ...
    WHEN 'patch' THEN
      SELECT jsonb_build_object(
        ...
        'offlineBehavior', settings.offline_behavior,
