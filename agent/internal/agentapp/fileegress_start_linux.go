...
// startFileEgress subscribes to fanotify FAN_CLOSE_WRITE events on removable and
// network-share mounts and reports files landing there via
// hb.SubmitFileEgressEvents. Non-fatal on init failure: the agent stays up, we
// just don't get file-egress detection. Mirrors the Windows starter's
// privilege-first ordering — fanotify needs CAP_SYS_ADMIN.
//
// The monitor self-gates on the heartbeat-delivered policy
// (hb.FileEgressConfig -> Enabled), so it is safe to start unconditionally on a
// privileged host; it stays idle until an enabled file_egress_policies row
// reaches this device.
//
...
func startFileEgress(ctx context.Context, hb *heartbeat.Heartbeat) <-chan struct{} {
	...
	if !privilege.IsRunningAsRoot() {
		log.Info("fileegress disabled: agent not running as root")
		close(done)
		return done
	}
